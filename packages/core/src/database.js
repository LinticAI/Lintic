...
export class SQLiteAdapter {
    ...
    backfillSqliteConversations() {
        const branches = this.db.prepare('SELECT id, session_id, name, created_at FROM session_branches ORDER BY created_at ASC, id ASC').all();
        for (const branch of branches) {
            let conversation = this.db.prepare("SELECT * FROM conversations WHERE branch_id = ? AND title = 'main' ORDER BY created_at ASC LIMIT 1").get(branch.id);
            if (!conversation) {
                const id = randomUUID();
                const timestamp = branch.created_at ?? Date.now();
                this.db.prepare(`INSERT INTO conversations (id, session_id, branch_id, title, archived, created_at, updated_at)
           VALUES (?, ?, ?, 'main', 0, ?, ?)`).run(id, branch.session_id, branch.id, timestamp, timestamp);
                conversation = {
                    id,
         ...
         SET conversation_id = ?
         WHERE session_id = ? AND branch_id = ? AND (conversation_id IS NULL OR conversation_id = '')`).run(conversation.id, branch.session_id, branch.id);
            this.db.prepare(`UPDATE replay_events
         SET conversation_id = ?
         WHERE session_id = ? AND branch_id = ? AND (conversation_id IS NULL OR conversation_id = '')`).run(conversation.id, branch.session_id, branch.id);
        }
    }
    ...
    createAssessmentLink(config) {
        this.db.prepare(`
      INSERT INTO assessment_links (
        id, token, url, prompt_id, candidate_email, created_at, expires_at, constraint_json
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(config.id, config.token, config.url, config.prompt_id, config.candidate_email, config.created_at, config.expires_at, JSON.stringify(config.constraint));
    ...
    getMainBranch(sessionId) {
        const row = this.db.prepare("SELECT * FROM session_branches WHERE session_id = ? AND name = 'main' LIMIT 1").get(sessionId);
        return Promise.resolve(row ? rowToSessionBranch(row) : null);
    }
    getBranch(sessionId, branchId) {
        const row = this.db.prepare('SELECT * FROM session_branches WHERE session_id = ? AND id = ? LIMIT 1').get(sessionId, branchId);
        return Promise.resolve(row ? rowToSessionBranch(row) : null);
    }
    listBranches(sessionId) {
        const rows = this.db.prepare('SELECT * FROM session_branches WHERE session_id = ? ORDER BY created_at ASC, id ASC').all(sessionId);
    ...
    getConversation(sessionId, conversationId) {
        const row = this.db.prepare('SELECT * FROM conversations WHERE session_id = ? AND id = ? LIMIT 1').get(sessionId, conversationId);
        return Promise.resolve(row ? rowToConversation(row) : null);
    }
    listConversations(sessionId, branchId) {
        const rows = this.db.prepare('SELECT * FROM conversations WHERE session_id = ? AND branch_id = ? ORDER BY updated_at DESC, created_at DESC, id DESC').all(sessionId, branchId);
    ...
    createConversation(config) {
        ...
        id, session_id, branch_id, title, archived, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, config.session_id, config.branch_id, config.title?.trim() || 'New chat', config.archived ? 1 : 0, createdAt, createdAt);
        return Promise.resolve({
            id,
    ...
    updateConversation(config) {
        const existing = this.db.prepare('SELECT * FROM conversations WHERE session_id = ? AND id = ? LIMIT 1').get(config.session_id, config.conversation_id);
        if (!existing) {
       ...
       WHERE id = ?`).run(nextTitle, nextArchived ? 1 : 0, updatedAt, config.conversation_id);
        return Promise.resolve({
            id: existing.id,
    ...
    createBranch(config) {
        ...
        this.db.prepare(`INSERT INTO conversations (
        id, session_id, branch_id, title, archived, created_at, updated_at
      ) VALUES (?, ?, ?, 'main', 0, ?, ?)`).run(mainConversationId, config.session_id, id, createdAt, createdAt);
      ...
      ORDER BY id ASC`).run(id, mainConversationId, config.session_id, config.parent_branch_id, parentConversation?.id ?? '', config.forked_from_sequence);
        this.db.prepare(`INSERT INTO replay_events (
      ...
      FROM replay_events
      WHERE session_id = ? AND branch_id = ? AND conversation_id = ? AND turn_sequence IS NOT NULL AND turn_sequence <= ?
      ORDER BY timestamp ASC, id ASC`).run(id, mainConversationId, config.session_id, config.parent_branch_id, parentConversation?.id ?? '', config.forked_from_sequence);
        const snapshot = this.db.prepare(`SELECT * FROM workspace_snapshots
       ...
       ORDER BY
         ...
         created_at DESC
       LIMIT 1`).get(config.session_id, config.parent_branch_id, config.forked_from_sequence);
        if (snapshot) {
            this.db.prepare(`INSERT INTO workspace_snapshots (
          id, session_id, branch_id, kind, turn_sequence, label, created_at, active_path, workspace_section, filesystem_json, mock_pg_json
        ) VALUES (?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?)`).run(randomUUID(), snapshot.session_id, id, snapshot.turn_sequence, snapshot.label, createdAt, snapshot.active_path, snapshot.workspace_section, snapshot.filesystem_json, snapshot.mock_pg_json);
        }
        return Promise.resolve({
            id,
            session_id: config.session_id,
    ...
    allocateTurnSequence(sessionId) {
        const row = this.db.prepare(`SELECT MAX(turn_sequence) AS max_turn_sequence FROM (
    ...
    `).run(sessionId, branchId, resolvedConversationId, turnSequence, role, content, tokenCount, Date.now());
        this.db.prepare('UPDATE conversations SET updated_at = ? WHERE id = ?').run(Date.now(), resolvedConversationId);
        return Promise.resolve();
    }
    ...
    getBranchMessages(sessionId, branchId, conversationId, options) {
        const includeRewound = options?.includeRewound ?? false;
        const rewoundFilter = includeRewound ? '' : 'AND rewound_at IS NULL';
        const rows = conversationId
            ? this.db.prepare(`SELECT * FROM messages WHERE session_id = ? AND branch_id = ? AND conversation_id = ? ${rewoundFilter} ORDER BY id ASC`).all(sessionId, branchId, conversationId)
            : this.db.prepare(`SELECT * FROM messages WHERE session_id = ? AND branch_id = ? ${rewoundFilter} ORDER BY id ASC`).all(sessionId, branchId);
    ...
    rewindMessages(sessionId, branchId, conversationId, afterTurnSequence) {
        this.db.prepare('UPDATE messages SET rewound_at = ? WHERE session_id = ? AND branch_id = ? AND conversation_id = ? AND turn_sequence > ?').run(Date.now(), sessionId, branchId, conversationId, afterTurnSequence);
        return Promise.resolve();
    }
    ...
    addBranchReplayEvent(sessionId, branchId, turnSequence, type, timestamp, payload, conversationId) {
        ...
        this.db.prepare('INSERT INTO replay_events (session_id, branch_id, conversation_id, turn_sequence, type, timestamp, payload) VALUES (?, ?, ?, ?, ?, ?, ?)').run(sessionId, branchId, resolvedConversationId, turnSequence, type, timestamp, JSON.stringify(payload));
        return Promise.resolve();
    }
    ...
    getBranchReplayEvents(sessionId, branchId, conversationId) {
        const rows = conversationId
            ? this.db.prepare('SELECT * FROM replay_events WHERE session_id = ? AND branch_id = ? AND conversation_id = ? ORDER BY timestamp ASC, id ASC').all(sessionId, branchId, conversationId)
            : this.db.prepare('SELECT * FROM replay_events WHERE session_id = ? AND branch_id = ? ORDER BY timestamp ASC, id ASC').all(sessionId, branchId);
        return Promise.resolve(rows.map((r) => ({
            id: r.id,
            session_id: r.session_id,
            branch_id: r.branch_id,
    ...
    listConversationContextAttachments(conversationId) {
        const rows = this.db.prepare('SELECT * FROM conversation_context_attachments WHERE conversation_id = ? ORDER BY created_at ASC, id ASC').all(conversationId);
        return Promise.resolve(rows.map(rowToContextAttachment));
    }
    replaceConversationContextAttachments(conversationId, attachments) {
        this.db.prepare('DELETE FROM conversation_context_attachments WHERE conversation_id = ?').run(conversationId);
        const createdAt = Date.now();
        for (const attachment of attachments) {
            this.db.prepare(`INSERT INTO conversation_context_attachments (
          id, conversation_id, kind, label, path, resource_id, source_conversation_id, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(randomUUID(), conversationId, attachment.kind, attachment.label, attachment.path ?? null, attachment.resource_id ?? null, attachment.source_conversation_id ?? null, createdAt);
        }
    ...
    listContextResources(sessionId, branchId) {
        const rows = this.db.prepare('SELECT * FROM context_resources WHERE session_id = ? AND branch_id = ? ORDER BY updated_at DESC, created_at DESC, id DESC').all(sessionId, branchId);
        return Promise.resolve(rows.map(rowToContextResource));
    }
    upsertContextResource(input) {
        const existing = this.db.prepare(`SELECT * FROM context_resources
       WHERE session_id = ? AND branch_id = ? AND kind = ? AND (
         (? IS NULL AND source_conversation_id IS NULL) OR source_conversation_id = ?
       )
       ORDER BY updated_at DESC
       LIMIT 1`).get(input.session_id, input.branch_id, input.kind, input.source_conversation_id ?? null, input.source_conversation_id ?? null);
        const timestamp = Date.now();
        if (existing) {
            this.db.prepare(`UPDATE context_resources
         SET title = ?, content = ?, updated_at = ?
         WHERE id = ?`).run(input.title, input.content, timestamp, existing.id);
            return Promise.resolve({
                id: existing.id,
                session_id: existing.session_id,
                branch_id: existing.branch_id,
        ...
        const id = randomUUID();
        this.db.prepare(`INSERT INTO context_resources (
        id, session_id, branch_id, kind, title, content, source_conversation_id, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.session_id, input.branch_id, input.kind, input.title, input.content, input.source_conversation_id ?? null, timestamp, timestamp);
        return Promise.resolve({
            id,
    ...
    async upsertWorkspaceSnapshot(input) {
        ...
        const existing = this.db.prepare(`SELECT id FROM workspace_snapshots
       WHERE session_id = ? AND branch_id = ? AND kind = ?
       ORDER BY created_at DESC LIMIT 1`).get(input.session_id, input.branch_id, input.kind);
        if (existing && input.kind === 'draft') {
            this.db.prepare(`UPDATE workspace_snapshots
         SET turn_sequence = ?, label = ?, created_at = ?, active_path = ?, workspace_section = ?, filesystem_json = ?, mock_pg_json = ?
         WHERE id = ?`).run(input.turn_sequence ?? null, input.label ?? null, createdAt, input.active_path ?? null, input.workspace_section ?? null, JSON.stringify(input.filesystem), JSON.stringify(input.mock_pg), existing.id);
            const row = this.db.prepare('SELECT * FROM workspace_snapshots WHERE id = ?').get(existing.id);
    ...
    createWorkspaceSnapshot(input) {
        ...
        this.db.prepare(`INSERT INTO workspace_snapshots (
        id, session_id, branch_id, kind, turn_sequence, label, created_at, active_path, workspace_section, filesystem_json, mock_pg_json
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.session_id, input.branch_id, input.kind, input.turn_sequence ?? null, input.label ?? null, createdAt, input.active_path ?? null, input.workspace_section ?? null, JSON.stringify(input.filesystem), JSON.stringify(input.mock_pg));
        return Promise.resolve({
            id,
            session_id: input.session_id,
            branch_id: input.branch_id,
    ...
    getWorkspaceSnapshot(sessionId, branchId, options = {}) {
        let row;
        if (options.turn_sequence !== undefined) {
            row = this.db.prepare(`SELECT * FROM workspace_snapshots
         WHERE session_id = ? AND branch_id = ? AND turn_sequence = ? AND (? IS NULL OR kind = ?)
         ORDER BY created_at DESC
         LIMIT 1`).get(sessionId, branchId, options.turn_sequence, options.kind ?? null, options.kind ?? null);
        }
        else if (options.kind) {
            row = this.db.prepare(`SELECT * FROM workspace_snapshots
         WHERE session_id = ? AND branch_id = ? AND kind = ?
         ORDER BY created_at DESC
         LIMIT 1`).get(sessionId, branchId, options.kind);
        }
        else {
            row = this.db.prepare(`SELECT * FROM workspace_snapshots
...
export class PostgresAdapter {
    ...
    async createSession(config) {
        ...
        await this.pool.query(`INSERT INTO session_branches (
        id, session_id, name, parent_branch_id, forked_from_sequence, created_at
      ) VALUES ($1, $2, 'main', NULL, NULL, $3)`, [branchId, id, now]);
        await this.pool.query(`INSERT INTO conversations (
        id, session_id, branch_id, title, archived, created_at, updated_at
      ) VALUES ($1, $2, $3, 'main', FALSE, $4, $5)`, [randomUUID(), id, branchId, now, now]);
        return { id, token };
    }
    async createAssessmentLink(config) {
        await this.initialize();
        await this.pool.query(`INSERT INTO assessment_links (
        id, token, url, prompt_id, candidate_email, created_at, expires_at, constraint_json
      ) VALUES (
        $1, $2, $3, $4, $5, $6, $7, $8
      )`, [
    ...
    async getMainBranch(sessionId) {
        await this.initialize();
        const result = await this.pool.query("SELECT * FROM session_branches WHERE session_id = $1 AND name = 'main' LIMIT 1", [sessionId]);
        return result.rows[0] ? rowToSessionBranch(normalizeSessionBranchRow(result.rows[0])) : null;
    }
    async getBranch(sessionId, branchId) {
        await this.initialize();
        const result = await this.pool.query('SELECT * FROM session_branches WHERE session_id = $1 AND id = $2 LIMIT 1', [sessionId, branchId]);
        return result.rows[0] ? rowToSessionBranch(normalizeSessionBranchRow(result.rows[0])) : null;
    }
    async listBranches(sessionId) {
        await this.initialize();
        const result = await this.pool.query('SELECT * FROM session_branches WHERE session_id = $1 ORDER BY created_at ASC, id ASC', [sessionId]);
    ...
    async getConversation(sessionId, conversationId) {
        await this.initialize();
        const result = await this.pool.query('SELECT * FROM conversations WHERE session_id = $1 AND id = $2 LIMIT 1', [sessionId, conversationId]);
        return result.rows[0] ? rowToConversation(normalizeConversationRow(result.rows[0])) : null;
    }
    async listConversations(sessionId, branchId) {
        await this.initialize();
        const result = await this.pool.query('SELECT * FROM conversations WHERE session_id = $1 AND branch_id = $2 ORDER BY updated_at DESC, created_at DESC, id DESC', [sessionId, branchId]);
    ...
    async createConversation(config) {
        ...
        await this.pool.query(`INSERT INTO conversations (
        id, session_id, branch_id, title, archived, created_at, updated_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, config.session_id, config.branch_id, title, config.archived ?? false, timestamp, timestamp]);
        return {
            id,
            session_id: config.session_id,
            branch_id: config.branch_id,
    ...
    async updateConversation(config) {
        await this.initialize();
        ...
        await this.pool.query(`UPDATE conversations SET title = $1, archived = $2, updated_at = $3 WHERE id = $4`, [title, archived, timestamp, config.conversation_id]);
        return {
            ...existing,
            title,
            archived,
    ...
    async createBranch(config) {
        ...
        await this.pool.query(`INSERT INTO session_branches (
        id, session_id, name, parent_branch_id, forked_from_sequence, created_at
      ) VALUES ($1, $2, $3, $4, $5, $6)`, [id, config.session_id, config.name, config.parent_branch_id, config.forked_from_sequence, createdAt]);
        const mainConversationId = randomUUID();
        await this.pool.query(`INSERT INTO conversations (
        id, session_id, branch_id, title, archived, created_at, updated_at
      ) VALUES ($1, $2, $3, 'main', FALSE, $4, $5)`, [mainConversationId, config.session_id, id, createdAt, createdAt]);
        await this.pool.query(`INSERT INTO messages (session_id, branch_id, conversation_id, turn_sequence, role, content, token_count, created_at)
       ...
       ORDER BY id ASC`, [id, mainConversationId, config.session_id, config.parent_branch_id, parentConversationId, config.forked_from_sequence]);
        await this.pool.query(`INSERT INTO replay_events (session_id, branch_id, conversation_id, turn_sequence, type, timestamp, payload)
       SELECT session_id, $1, $2, turn_sequence, type, timestamp, payload
       FROM replay_events
       WHERE session_id = $3 AND branch_id = $4 AND conversation_id = $5 AND turn_sequence IS NOT NULL AND turn_sequence <= $6
       ORDER BY timestamp ASC, id ASC`, [id, mainConversationId, config.session_id, config.parent_branch_id, parentConversationId, config.forked_from_sequence]);
        const snapshotResult = await this.pool.query(`SELECT * FROM workspace_snapshots
       WHERE session_id = $1 AND branch_id = $2 AND (
         kind = 'draft' OR (turn_sequence IS NOT NULL AND turn_sequence <= $3)
       )
       ORDER BY CASE WHEN kind = 'draft' THEN 0 ELSE 1 END, created_at DESC
       LIMIT 1`, [config.session_id, config.parent_branch_id, config.forked_from_sequence]);
        const snapshot = snapshotResult.rows[0];
        if (snapshot) {
            const normalized = normalizeWorkspaceSnapshotRow(snapshot);
            await this.pool.query(`INSERT INTO workspace_snapshots (
          id, session_id, branch_id, kind, turn_sequence, label, created_at, active_path, workspace_section, filesystem_json, mock_pg_json
    ...
    async allocateTurnSequence(sessionId) {
        await this.initialize();
        const result = await this.pool.query(`SELECT MAX(turn_sequence) AS max_turn_sequence FROM (
       ...
       ) AS turn_sequences`, [sessionId]);
        return Number(result.rows[0]?.max_turn_sequence ?? 0) + 1;
    }
    async addBranchMessage(sessionId, branchId, turnSequence, role, content, tokenCount, conversationId) {
        await this.initialize();
        ...
        await this.pool.query(`INSERT INTO messages (session_id, branch_id, conversation_id, turn_sequence, role, content, token_count, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [sessionId, branchId, resolvedConversationId, turnSequence, role, content, tokenCount, Date.now()]);
    ...
    async getBranchMessages(sessionId, branchId, conversationId, options) {
        await this.initialize();
        const includeRewound = options?.includeRewound ?? false;
        const rewoundFilter = includeRewound ? '' : 'AND rewound_at IS NULL';
        const result = conversationId
            ? await this.pool.query(`SELECT * FROM messages WHERE session_id = $1 AND branch_id = $2 AND conversation_id = $3 ${rewoundFilter} ORDER BY id ASC`, [sessionId, branchId, conversationId])
            : await this.pool.query(`SELECT * FROM messages WHERE session_id = $1 AND branch_id = $2 ${rewoundFilter} ORDER BY id ASC`, [sessionId, branchId]);
    ...
    async rewindMessages(sessionId, branchId, conversationId, afterTurnSequence) {
        await this.initialize();
        await this.pool.query('UPDATE messages SET rewound_at = $1 WHERE session_id = $2 AND branch_id = $3 AND conversation_id = $4 AND turn_sequence > $5', [Date.now(), sessionId, branchId, conversationId, afterTurnSequence]);
    }
    ...
    async addBranchReplayEvent(sessionId, branchId, turnSequence, type, timestamp, payload, conversationId) {
        await this.initialize();
        const resolvedConversationId = conversationId ?? (await this.getMainConversation(sessionId, branchId))?.id;
        if (!resolvedConversationId) {
            throw new Error(`Main conversation not found for branch ${branchId}`);
        ...
        await this.pool.query(`INSERT INTO replay_events (session_id, branch_id, conversation_id, turn_sequence, type, timestamp, payload)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [sessionId, branchId, resolvedConversationId, turnSequence, type, timestamp, JSON.stringify(payload)]);
    }
    async getBranchReplayEvents(sessionId, branchId, conversationId) {
        await this.initialize();
        const result = conversationId
            ? await this.pool.query('SELECT * FROM replay_events WHERE session_id = $1 AND branch_id = $2 AND conversation_id = $3 ORDER BY timestamp ASC, id ASC', [sessionId, branchId, conversationId])
            : await this.pool.query('SELECT * FROM replay_events WHERE session_id = $1 AND branch_id = $2 ORDER BY timestamp ASC, id ASC', [sessionId, branchId]);
        return result.rows.map((row) => ({
            id: Number(row.id),
            session_id: row.session_id,
            branch_id: row.branch_id,
    ...
    async listConversationContextAttachments(conversationId) {
        await this.initialize();
        const result = await this.pool.query('SELECT * FROM conversation_context_attachments WHERE conversation_id = $1 ORDER BY created_at ASC, id ASC', [conversationId]);
        return result.rows.map((row) => rowToContextAttachment(normalizeContextAttachmentRow(row)));
    }
    async replaceConversationContextAttachments(conversationId, attachments) {
        await this.initialize();
        await this.pool.query('DELETE FROM conversation_context_attachments WHERE conversation_id = $1', [conversationId]);
        const timestamp = Date.now();
        for (const attachment of attachments) {
            await this.pool.query(`INSERT INTO conversation_context_attachments (
          id, conversation_id, kind, label, path, resource_id, source_conversation_id, created_at
    ...
    async listContextResources(sessionId, branchId) {
        await this.initialize();
        const result = await this.pool.query('SELECT * FROM context_resources WHERE session_id = $1 AND branch_id = $2 ORDER BY updated_at DESC, created_at DESC, id DESC', [sessionId, branchId]);
        return result.rows.map((row) => rowToContextResource(normalizeContextResourceRow(row)));
    }
    async upsertContextResource(input) {
        await this.initialize();
        const timestamp = Date.now();
        const existing = await this.pool.query(`SELECT * FROM context_resources
       ...
       ORDER BY updated_at DESC
       LIMIT 1`, [input.session_id, input.branch_id, input.kind, input.source_conversation_id ?? null]);
        if (existing.rows[0]) {
            const normalized = normalizeContextResourceRow(existing.rows[0]);
            await this.pool.query(`UPDATE context_resources SET title = $1, content = $2, updated_at = $3 WHERE id = $4`, [input.title, input.content, timestamp, normalized.id]);
            return {
                id: normalized.id,
        ...
        const id = randomUUID();
        await this.pool.query(`INSERT INTO context_resources (
        id, session_id, branch_id, kind, title, content, source_conversation_id, created_at, updated_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [id, input.session_id, input.branch_id, input.kind, input.title, input.content, input.source_conversation_id ?? null, timestamp, timestamp]);
        return {
            id,
            session_id: input.session_id,
            branch_id: input.branch_id,
    ...
    async upsertWorkspaceSnapshot(input) {
        await this.initialize();
        if (input.kind === 'draft') {
            const existing = await this.pool.query(`SELECT id FROM workspace_snapshots
         WHERE session_id = $1 AND branch_id = $2 AND kind = 'draft'
         ORDER BY created_at DESC
         LIMIT 1`, [input.session_id, input.branch_id]);
            if (existing.rows[0]?.id) {
                const createdAt = input.created_at ?? Date.now();
                await this.pool.query(`UPDATE workspace_snapshots
           ...
           WHERE id = $8`, [
                ...
                const rowResult = await this.pool.query('SELECT * FROM workspace_snapshots WHERE id = $1', [existing.rows[0].id]);
    ...
    async getWorkspaceSnapshot(sessionId, branchId, options = {}) {
        await this.initialize();
        let result;
        if (options.turn_sequence !== undefined) {
            result = await this.pool.query(`SELECT * FROM workspace_snapshots
         WHERE session_id = $1 AND branch_id = $2 AND turn_sequence = $3 AND ($4::text IS NULL OR kind = $4)
         ORDER BY created_at DESC
         LIMIT 1`, [sessionId, branchId, options.turn_sequence, options.kind ?? null]);
        }
        else if (options.kind) {
            result = await this.pool.query(`SELECT * FROM workspace_snapshots
         WHERE session_id = $1 AND branch_id = $2 AND kind = $3
         ORDER BY created_at DESC
         LIMIT 1`, [sessionId, branchId, options.kind]);
        }
        else {
            result = await this.pool.query(`SELECT * FROM workspace_snapshots
    ...
    async backfillPostgresConversations() {
        const branchesResult = await this.pool.query('SELECT * FROM session_branches ORDER BY created_at ASC, id ASC');
        for (const row of branchesResult.rows) {
            const branch = normalizeSessionBranchRow(row);
            let conversationResult = await this.pool.query("SELECT * FROM conversations WHERE branch_id = $1 AND title = 'main' ORDER BY created_at ASC LIMIT 1", [branch.id]);
            if (!conversationResult.rows[0]) {
                const id = randomUUID();
                await this.pool.query(`INSERT INTO conversations (id, session_id, branch_id, title, archived, created_at, updated_at)
           VALUES ($1, $2, $3, 'main', FALSE, $4, $5)`, [id, branch.session_id, branch.id, branch.created_at, branch.created_at]);
                conversationResult = await this.pool.query('SELECT * FROM conversations WHERE id = $1', [id]);
            }
            const conversation = normalizeConversationRow(conversationResult.rows[0]);
            await this.pool.query(`UPDATE messages
         SET conversation_id = $1
         WHERE session_id = $2 AND branch_id = $3 AND (conversation_id IS NULL OR conversation_id = '')`, [conversation.id, branch.session_id, branch.id]);
            await this.pool.query(`UPDATE replay_events
         SET conversation_id = $1
         WHERE session_id = $2 AND branch_id = $3 AND (conversation_id IS NULL OR conversation_id = '')`, [conversation.id, branch.session_id, branch.id]);
        }
    }
...
function normalizeAssessmentLinkRow(row) {
    return {
        ...row,
        created_at: Number(row.created_at),
...
function normalizeSessionBranchRow(row) {
    return {
        ...row,
        forked_from_sequence: row.forked_from_sequence === null || row.forked_from_sequence === undefined
            ? null
...
function normalizeContextAttachmentRow(row) {
    return {
        ...row,
        created_at: Number(row.created_at),
    };
}
function normalizeContextResourceRow(row) {
    return {
        ...row,
        created_at: Number(row.created_at),
        updated_at: Number(row.updated_at),
...
function normalizeWorkspaceSnapshotRow(row) {
    return {
        ...row,
        turn_sequence: row.turn_sequence === null || row.turn_sequence === undefined ? null : Number(row.turn_sequence),
        created_at: Number(row.created_at),
    };
}
...
function rowToSessionBranch(row) {
    const branch = {
        id: row.id,
        session_id: row.session_id,
        name: row.name,
...
function rowToContextAttachment(row) {
    return {
        id: row.id,
        conversation_id: row.conversation_id,
        kind: row.kind,
...
function rowToContextResource(row) {
    return {
        id: row.id,
        session_id: row.session_id,
        branch_id: row.branch_id,
...
function rowToWorkspaceSnapshot(row) {
    const snapshot = {
        id: row.id,
        session_id: row.session_id,
        branch_id: row.branch_id,
        kind: row.kind,
        created_at: row.created_at,
        filesystem: JSON.parse(row.filesystem_json),
        mock_pg: JSON.parse(row.mock_pg_json),
    };
    ...
    if (row.workspace_section) {
        snapshot.workspace_section = row.workspace_section;
    }
    return snapshot;
}
function rowToAssessmentLink(row) {
    const link = {
        id: row.id,
        token: row.token,
        url: row.url,
